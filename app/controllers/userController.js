...
const Users = {
  async signUpUsers(req, res) {
    ...
    try {
      ...
      res
        .status(500)
        .json({ status: 500, message: 'Account cannot be created!' });
    }
  },
  ...
  async userSignIn(req, res) {
    try {
      ...
      if (!rows[0]) {
        res.status(406).json({ error: 'Incorrect email address' });
      }
      ...
      res
        .status(401)
        .json({ status: 401, message: 'You are denied access!' });
    }
  },
  ...
  async forgotPassword(req, res) {
    try {
      ...
      if (!rows[0]) {
        res
          .status(404)
          .json({ error: 'There is no account with this email address' });
      }
      ...
      res
        ...
        .json({
          status: 200,
          message: `Password reset instruction was sent to ${email}`,
        });
    } catch (error) {
      res
        .status(500)
  ...
  async resetPassword(req, res) {
    try {
      ...
      res
        .status(200)
        .json({ status: 200, message: 'Password was successfully reset' });
    } catch (error) {
      res
        .status(500)
        .json({ status: 500, message: 'Something went wrong!' });
